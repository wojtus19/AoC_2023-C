...
int FindSourceIndex(vdSources_t* sources, char* id, int count)
{
    for (int i = 0; i < count; i++)
    {
        if (strcmp(sources[i].source, id) == 0)
            return i;
    }
    return NOT_FOUND;
}

lld_t calculateGCD(lld_t n1, lld_t n2)
{
    if (n2 == 0)
        return n1;
    else
...
lld_t calculateLCM(lld_t n1, lld_t n2)
{
    lld_t gcd = calculateGCD(n1, n2);
    return (n1 * n2) / gcd;
}

bool AllFilled(vdSources_t* vdSources, int count)
{
    for (int i = 0; i < count; i++)
    {
        if (vdSources[i].iterations == 0)
            return false;
    }
    return true;
}
...
int main()
{
    ...
    char rxSource[] = "vd";
    vdSources_t vdSources[MAX_VD_SOURCES];
    int sourcesCnt = 0;
    for (int i = 0; i < modulesVector.modulesAmount; i++)
    {
        if (modulesVector.modules[i].type == CONJUNCTION)
        {
            for (int j = 0; j < modulesVector.modules[i].destinationModulesAmount; j++)
            {
                if (strcmp(modulesVector.modules[i].destinationModulesIds[j], rxSource) == 0)
                {
                    strcpy(vdSources[sourcesCnt].source, modulesVector.modules[i].id);
                    sourcesCnt++;
                }
            }
        }
    ...
    for (int i = 0; i < MAX_ITER; i++)
    {
        ...
        while (qAmount != qCurrIdx)
        {
            ...
            else // & - CONJUNCTION
            {
                ...
                if (sendPulse == HIGH)
                {
                    int sourcesIdx = FindSourceIndex(vdSources, modulesVector.modules[currModuleIdx].id, sourcesCnt);
                    if (sourcesIdx != NOT_FOUND)
                    {
                        vdSources[sourcesIdx].iterations = i + 1;
                        if (AllFilled(vdSources, sourcesCnt))
                            goto found;
                    }
                }
                for (int j = 0; j < modulesVector.modules[currModuleIdx].destinationModulesAmount; j++)
...
found:
    ...
    lld_t iterations = vdSources[0].iterations;
    for (int i = 1; i < sourcesCnt; i++)
    {
        iterations = calculateLCM(iterations, vdSources[i].iterations);
    }
